...
def get_times(years):
    now = datetime.utcnow()
    prev_year = datetime(now.year - years, now.month, now.day, now.hour,
                         now.minute, now.second, now.microsecond, now.tzinfo)
    return now, prev_year
    ...
    while best > prev_year:
        ...
        if best < prev_year:
            ...
            while index >= 0:
                ...
                now, prev_year = get_times(years)
                sleep_seconds = (
                    next_tweet.created_at - prev_year).total_seconds()
            ...
            user_timeline = tweepy.models.ResultSet()
            max_id = None
            best, prev_year = get_times(years)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="It's like Twitshift, but for people who prefer Twolde",
